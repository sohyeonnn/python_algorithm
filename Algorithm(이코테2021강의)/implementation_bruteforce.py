#구현(시물레이션과 완전탐색(brute force))
'''
#구현;
- 머리속에있는 알고리즘을 소스코드로 바꾸는 과정
...
#브루트포스(brute force)
- 가능한 모든 조합을 다 탐색한다.(답이 나올때까지)
-답을 무조건 찾을수는 있으나, 답을 찾는데, 비용이 너무 크다는 단점이 있음.
=> 시간복잡도가 크다.

- 일반적으로 시간복잡도는 O(n!) or O(2^n)

- 비교대상만큼 for문을 작성해야 하므로 가독성도 떨어진다.
'''
